...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope rigor-cli
...
scope:** the default personal scope is **private** (`--scope rigor-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `rigor-cli`). Private records are served only to a recall in the same
