...
rigor-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. rigor-cli
...
#   - consumer-identifying prose: `guildmaster` → `rigor-cli` (NOT
...
If a re-sync would lose a rigor-cli adaptation, lift the change
...
#     → `which rigor-cli vendors from guildmaster`
